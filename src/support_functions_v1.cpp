...
void averageImage(std::string filename, cv::Mat &finalImage, int numImages)
{
    ...
    img = imread((filename + index), cv::IMREAD_ANYCOLOR);
    imageSize = img.size();
    ...
    for (idx = 1; idx < numImages; idx++)
    {
        ...
        img = cv::imread((filename + index), cv::IMREAD_ANYCOLOR);
        cv::add(img, imgAvg, imgAvg, Mat(), CV_64FC3);
    }
...
void doSomething(std::string filename, cv::Mat &img)
{
    ...
    tempIn = imread((filename + "Depth_Map_132-136-1.0-avg4.png"), cv::IMREAD_ANYCOLOR);
