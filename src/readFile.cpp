...
void readFile(Mat &infocusImage, Mat &defocusImage, string f1, string f2, int type, int num)
{
    ...
    switch (type)
    {
        case 0:     // two individual files
            ...
            infocusImage = cv::imread(f1, cv::IMREAD_COLOR);
            defocusImage = cv::imread(f2, cv::IMREAD_COLOR);
        ...
        case 3:         // time median of files
            ...
            img1 = cv::imread(f1, cv::IMREAD_COLOR);
            imageSize = img1.size();
            ...
            for (idx = 0; idx < num; idx++)
            {
                ...
                medMat_I[idx] = cv::imread(file1, cv::IMREAD_COLOR);
                medMat_O[idx] = cv::imread(file2, cv::IMREAD_COLOR);
        ...
        case 4:         // run histogram on dark image
            ...
            img1 = imread(f1, cv::IMREAD_COLOR);
